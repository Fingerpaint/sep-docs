...
Each document should have a ``main'' \texttt{.tex}-file that \verb|\input|s separate \texttt{.tex}-files that each contain a chapter (or appendix). This enables project members to work on the same document more efficiently, as working on different chapters will not cause any merge conflicts.

So, a standard ``main'' \texttt{.tex}-file should look like as is shown in figure \ref{fig:exampledocument}.
