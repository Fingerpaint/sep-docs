...
2 & Missing & A description of the design descision to compress with zip is needed. In addition, the alternative (lwz compression) should be described.\\
\end{longtable}
...
\feedbackTableStart{\hugo{}}{0.0}
3.0 & 		Style & 	``with this, we mean'' $\rightarrow$ ``This means...''\\
3.1 & 		Typo & 		``with this module are done'' $\rightarrow$ ``is done''\\
3.1 & 		Typo  & 	``In the above, all meta-parameters are not needed to make'' $\rightarrow$ ``Not all meta-parameters of the above table are needed...''\\
...
\feedbackTableStart{\roel{}}{0.1}
5.1.2 & Question & In \texttt{Data}, it is stated that internal data results may have to be duplicated for intermediate results, but intermediate results are not implemented.\\
\end{longtable}
...
\subsection{Internal reviews}
\feedbackTableStart{\roel{}}{0.1}
6 & Question & \emph{The operating system requirement for the application is \dots or some up-to-date Linux distribution} I find some up-to-date distribution a rather vague description for a linux version.\\
\end{longtable} 

\feedbackTableStart{\hugo{}}{0.0}
6 (client device) & Question & 	So if you want to run FINGERPAINT on your smartphone, your smartphone needs $\rightarrow$ 1 GB RAM? This seems a lot.\\
\end{longtable}
